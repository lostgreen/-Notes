...
Dave收到两个消息
Tue X:2,Y:1  with Ben（新版本）
Thu X:1,Y:1   from Cathy （**该消息产生较早，但到达Dave较晚**）
发现Tue是Thu的祖先，所以抛弃祖先，选择Ben的消息，这样就丢失了Cathy给他的消息。
![传播时延](image-3.png)
...
Riak 的向量时钟剪枝机制通过 **small_vclock**、**big_vclock**、**young_vclock** 和 **old_vclock** 四个参数，结合向量时钟的长度和存活时间，来有效控制向量时钟的增长，同时尽量保留系统所需的版本历史。这种设计为向量时钟的管理提供了灵活性和可控性。

---

### **四个参数的作用**

1. **small_vclock** 和 **big_vclock**（基于长度）：
   - **small_vclock**：设定一个下限，向量时钟的长度小于这个值时不会被剪枝。目的是确保小范围内的版本冲突仍可被有效地追踪。
   - **big_vclock**：设定一个上限，当向量时钟的长度超过这个值时会被标记为需要剪枝。此参数用于避免向量时钟长度无限增长。

2. **young_vclock** 和 **old_vclock**（基于时间戳）：
   - **young_vclock**：设定一个时间下限，对于较新的向量时钟（小于此时间范围的），系统认为其仍然活跃，不会被剪枝。
   - **old_vclock**：设定一个时间上限，存活时间超过此值的向量时钟会被剪枝，认为这些版本冲突的可能性已较低。
...
现在我们有一个向量时钟 \( VC = \{A: 3, B: 4, C: 2\} \)，并在以下场景中处理：

1. **长度检查**：
   - 当前长度为 3（条目数：\( A, B, C \)），小于 **small_vclock** 的值 5，因此不会剪枝。

2. **长度超过上限**：
   - 假设向量时钟增长到 \( \{A: 6, B: 5, C: 4, D: 2, E: 3, F: 1\} \)，长度为 6，介于 **small_vclock** 和 **big_vclock** 之间，不会立即被剪枝。
   - 如果长度达到 11，则超出 **big_vclock** 的上限 10，系统会触发剪枝机制，移除较旧的条目（如 \( F \)）。

3. **时间检查**：
   - 如果向量时钟的创建时间在 20 秒前，则小于 **young_vclock**，不会被剪枝。
   - 如果时间达到 400 秒，则超过 **old_vclock**，即使长度未达到 **big_vclock**，也可能被剪枝。

**Problem**:特殊情况带来的问题：一个客户端保持了一个很久之前的向量时钟，然后继承该向量时钟并提交了一个数据，此时会产生问题（因为服务器已经没有这个很久之前的向量时钟信息了，可能已经被剪枝掉了），所以客户端提交的此次数据，在服务端无法找到一个祖先，此时Riak剪枝系统会创建一个sibling
